 ...
 * in a softirq context on older kernels but on newer kernels
 * (hrtimer / workqueue-based) sleeping is safe. More importantly,
 * get_task_mm() inside the callback can sleep, so a spinlock would
 * be incorrect. A mutex allows safe sleeping while holding the lock.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /* list_for_each_entry_safe allows removal during iteration */
